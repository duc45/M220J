...
public class MovieDao extends AbstractMFlixDao {
    ...
    public Document getMovie(String movieId) {
        ...
        match = Aggregates.match( Filters.expr( eq ));
        Bson sort = Aggregates.sort( Sorts.descending("date"));

        Bson lookup = Aggregates.lookup("comments", Arrays.asList(let), Arrays.asList(match, sort), "comments");
    ...
    public List<Document> getMoviesByCountry(String... country) {
        ...
        queryFilter.append("countries", new Document("$all", Arrays.asList(country)));
    ...
    public List<Document> getMoviesByGenre(String sortKey, int limit, int skip, String... genres) {
        ...
        moviesCollection.find(castFilter).sort(Sorts.orderBy(sorts)).limit(limit).skip(skip).iterator()
        .forEachRemaining(movies::add);
        return movies;
    }
